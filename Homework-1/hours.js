const SUN = `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-brightness-high" viewBox="0 0 16 16">
<path d="M8 11a3 3 0 1 1 0-6 3 3 0 0 1 0 6zm0 1a4 4 0 1 0 0-8 4 4 0 0 0 0 8zM8 0a.5.5 0 0 1 .5.5v2a.5.5 0 0 1-1 0v-2A.5.5 0 0 1 8 0zm0 13a.5.5 0 0 1 .5.5v2a.5.5 0 0 1-1 0v-2A.5.5 0 0 1 8 13zm8-5a.5.5 0 0 1-.5.5h-2a.5.5 0 0 1 0-1h2a.5.5 0 0 1 .5.5zM3 8a.5.5 0 0 1-.5.5h-2a.5.5 0 0 1 0-1h2A.5.5 0 0 1 3 8zm10.657-5.657a.5.5 0 0 1 0 .707l-1.414 1.415a.5.5 0 1 1-.707-.708l1.414-1.414a.5.5 0 0 1 .707 0zm-9.193 9.193a.5.5 0 0 1 0 .707L3.05 13.657a.5.5 0 0 1-.707-.707l1.414-1.414a.5.5 0 0 1 .707 0zm9.193 2.121a.5.5 0 0 1-.707 0l-1.414-1.414a.5.5 0 0 1 .707-.707l1.414 1.414a.5.5 0 0 1 0 .707zM4.464 4.465a.5.5 0 0 1-.707 0L2.343 3.05a.5.5 0 1 1 .707-.707l1.414 1.414a.5.5 0 0 1 0 .708z"/>
</svg>`

const MOON = `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-moon-fill" viewBox="0 0 16 16">
<path d="M6 .278a.768.768 0 0 1 .08.858 7.208 7.208 0 0 0-.878 3.46c0 4.021 3.278 7.277 7.318 7.277.527 0 1.04-.055 1.533-.16a.787.787 0 0 1 .81.316.733.733 0 0 1-.031.893A8.349 8.349 0 0 1 8.344 16C3.734 16 0 12.286 0 7.71 0 4.266 2.114 1.312 5.124.06A.752.752 0 0 1 6 .278z"/>
</svg>`

let nameLast = prompt("Lütfen Adınızı Giriniz: ")
let name = document.querySelector("#name")

// Girilen ismin ilk harfi büyük yapıldı:
nameLast = `${nameLast[0].toUpperCase()}${nameLast.slice(1)}`
name.innerHTML = nameLast

// Saatin ayarlanması:
function time() {
    let clock = document.getElementById("hour")

    var now = new Date();
    let hour = now.getHours(); 
    let minute = now.getMinutes(); 
    let second = now.getSeconds(); 
    let time = `${hour}:${minute}:${second}`; 

    clock.innerHTML = time;

}
setInterval(time, 1000);
time();

// Gün ayarlanması:
let dateNow = document.querySelector(".day")
const date = new Date();
var days = ['Pazar', 'Pazartesi', 'Salı', 'Çarşamba', 'Perşembe', 'Cuma', 'Cumartesi'];
dateNow.innerHTML = `${days[date.getDay()]} `

//tarihin alınması:
let fullDate = document.getElementById("fullDate")
let now = new Date();
let day = now.getDate()
let month = now.getMonth() + 1; 
let year = now.getFullYear(); 
let hour = now.getHours(); 

fullDate.innerHTML = `${day}/${month}/${year}`

// btn atamaları:
let dark = document.getElementById("btn")
let mood = dark.innerHTML = MOON

function getLight() {
    /*
    //yazı rengi degistirildi
    let text = document.getElementById("mainDiv");
    text.classList.remove("text-light");
    text.classList.add("text-dark");
    dark.innerHTML = MOON;
    */
   
   var body = document.body;
   body.classList.toggle("dark-mode");
}
